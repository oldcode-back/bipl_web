...
const Upcoming_cards = ({ name, image, location, rate, link }) => {
  return (
    <div className="mb-8">
      {" "}
...
const CardGrid = () => {
  ...
  useEffect(() => {
    const handleUpcomingData = async () => {
      try {
        const response = await axios.get(`${BackendAPI}upcomingData`, {
          params: {
            state: state,
            city: city,
          },
        ...
        if (response.data.success) {
          setUpcomingData(response.data.UpcomingData);
        } else {
  ...
  return (
    <div className="bg-gray-50 pb-32">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 ml-44 mr-32 ">
        {UpcomingData.map((restaurant, index) => (
          <Upcoming_cards
            key={index}
