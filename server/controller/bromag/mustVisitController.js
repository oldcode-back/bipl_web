...
const getMustVisitBannerToUpdate = async (req, res) => {
  try {
    const bannerId = req.params.bannerId;
    const BannerData = await MustVisitBanner.findOne({
      _id: bannerId,
    });
...
const updateMustVisitBannerData = async (req, res) => {
  try {
    console.log(req.body, "data from body");
    const { state, city, banner } = req.body;

    const bannerId = req.params.bannerId;
    const Banner = await MustVisitBanner.findOne({ _id: bannerId });
    ...
    console.log(updatedData, "updated data");
    const updatedPartner = await MustVisitBanner.findOneAndUpdate(
      { _id: bannerId },
      { $set: updatedData },
      { new: true }
    );
...
module.exports = {
  ...
  updateMustVisitData,
  updateMustVisitBannerData,
  getMustVisitBannerToUpdate,
};
