...
const getUpcomingBannerToUpdate = async (req, res) => {
  try {
    const bannerId = req.params.bannerId;
    const BannerData = await UpcomingBanner.findOne({
      _id: bannerId,
    });
...
const updateUpcomingBannerData = async (req, res) => {
  try {
    console.log(req.body, "data from body");
    const { state, city, banner } = req.body;

    const bannerId = req.params.bannerId;
    const Banner = await UpcomingBanner.findOne({ _id: bannerId });
    ...
    console.log(updatedData, "updated data");
    const updatedPartner = await UpcomingBanner.findOneAndUpdate(
      { _id: bannerId },
      { $set: updatedData },
      { new: true }
    );
...
module.exports = {
  ...
  getUpcomingToUpdate,
  updateUpcomingData,
  getUpcomingBannerToUpdate,
  updateUpcomingBannerData,
};
